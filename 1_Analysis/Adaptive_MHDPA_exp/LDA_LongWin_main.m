...
function LDA_LongWin_main(adaptmode,fusionmode)
...
Acc_val = [];
    ...
    for Nf = 1:fix(length(chlist)/2)
        ...
        X_ = [];
        for tr = 1:Nval
            X_(:,:,tr) = param_all.DSP.W(:,1: param.DSP.nf)'*val_temp(:,param.DSP.window,tr);
        end
        [~,sc_] = predict(param_all.trD.mdl,reshape(X_,[],size(X_,3))');
        
        Acc_val(Nf,:) = sum(sc_(:,end)>0.5 == Label_val)/length(Label_val);
    ...
    for Nf = 1:fix(length(chlist)/2)
        ...
        param.DSP.nf = Nf;%fix(length(chlist)/2);        
        yy = [];  param_temp = [];
        for ti = 1:Ntime
...
%                     Feat_te(:,:,ti,tt) = Y_;
            ...
            [upcheck2,up2,Posterior2,trs2,D2] = checkupdate(param_all.trD.score, [] ,[],c2,[]);
...
cd(fpathanalysis)
mkdir('adaptiveMWHDPA_LDA_LongWin_NoThre_VarNf_VarUC')
save(['adaptiveMWHDPA_LDA_LongWin_NoThre_VarNf_VarUC\',adaptmode,'_',fusionmode,'.mat'],...
     'scores1','scores2','outputs1','outputs2','outputs','answers',...
     'DSPs','BadChs','Temporal','LDA','Updated1','Updated2','UpDistance1','UpDistance2','DSPtotal','Lambdas')
