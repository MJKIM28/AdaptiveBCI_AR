...
% 2. fixed multi
LDA_multiOnly_main_func2('fixed','NotFusion',Kfold)

% 3. adaptive multi

LDA_multiOnly_main_func2('adaptive','NotFusion',Kfold)

% 4. adaptive-fixed fusion multi
LDA_multiOnly_main_func2('adaptive','fusion',Kfold)
...
% 
% f_m = load('NewFunc_adaptiveMWHDPA_LDA_MultiONly_NoThre_VarNf_VarUC_0.08\fixed_NotFusion.mat');
% [~,res1] = max(f_m.outputs1);
% Acc_f_m = squeeze(sum(squeeze(res1) ==f_m.answers)./sum(~isnan(f_m.answers)))'*100; % fixed multi
% 

a_nf = load('NewFunc_adaptiveMWHDPA_LDA_MultiONly_NoThre_VarNf_VarUC_0.06\adaptive_NotFusion.mat');
[~,res1] = max(a_nf.outputs1);
Acc_a_nf = squeeze(sum(squeeze(res1) ==a_nf.answers)./sum(~isnan(a_nf.answers)))'*100; % adapt not fusion

[~,res2] = max(a_nf.outputs2);
Acc_f_m = squeeze(sum(squeeze(res2) ==a_nf.answers)./sum(~isnan(a_nf.answers)))'*100; % adapt not fusion

a_f = load('NewFunc_adaptiveMWHDPA_LDA_MultiONly_NoThre_VarNf_VarUC_0.06\adaptive_fusion.mat');
Acc_a_f = squeeze(sum(a_f.outputs ==a_f.answers)./sum(~isnan(a_f.answers)))'*100; % adapt fusion

LambdasLDA = NaN(8,15,7,10);
for s = 1:10
    for c = 1:7
        for t= 1:15
            mdl = a_f.LDA{t,c,s};
            if ~isempty(mdl)
                for ti = 1:8
                LambdasLDA(ti,t,c,s) = mdl(ti).lambda;
                end
            end
        end
    end
...
for s = 1:Nsub
    ...
    a3 = plot(Acc_a_f_ulr(s,:)','Color',colors(4,:),'marker','diamond','LineWidth',2,'MarkerFaceColor',colors(4,:),'LineStyle','-');
    plot([1.5 1.5],[0 105],'Color',[0.3 0.3 0.3],'LineStyle','--')
plot([6.5 6.5],[0 105],'Color',[0.3 0.3 0.3],'LineStyle','--')
