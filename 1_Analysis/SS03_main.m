...
scores2 = NaN(40,1,Ntrial,Ncon,Nsub);
...
if s <5
    ...
    param.DSP.nf = 4;%fix(length(chlist)/2);
...
%         mdl{ti,1} = fitcdiscr(X, Y);

        mdl_mj{ti} = getLDA(X,Y);
        %         mdl_mj{ti}.Sigma = mdl{ti}.Sigma;
        %         mdl_mj{ti}.Mu = mdl{ti}.Mu;
        %         mdl_mj{ti}.invSigma = inv(mdl_mj{ti}.Sigma);
        %         mdl_mj{ti}.Prior = mdl{ti}.Prior;
        ...
        %          [~,sc_] = predict(mdl{ti},reshape(X_,[],size(X_,3))');
    ...
    importance = abs(model.Coefficients.Estimate(2:end)); % 첫 번째 계수는 절편이므로 제외
        ...
        for tr = trials
            ...
            for tt = 1:size(test_temp,3)
                for ti = 1:Ntime_new
                    ...
                    Y_ = param_temp{ti}.DSP.W(:,1: param.DSP.nf)'*test_temp(:,windowind,tt);
                    Feat_te(:,:,ti,tt) = Y_;
...
%             threshold1 = mean(threshold1_tmp);
            ...
            [upcheck2,up2,Posterior2,trs2,D2] = checkupdate(param_all.trD.score, [] ,threshold2,c2,[]);

            threshold2_tmp = [threshold2_tmp D2(c2)];
            if length(threshold2_tmp) > 10
                threshold2_tmp(1) = [];
            end
%             threshold2 = mean(threshold2_tmp);
            ...
            scores1(:,:,t,c,s) = rho;
            scores2(:,:,t,c,s) = param_all.trD.score(:);
...
save(['adaptiveSPHDCA\SS03_exp_adaptiveSPHDCA_LDA_PMean_l1_',num2str(lambda1),...
    '_l2_',num2str(lambda2),'_thres1_',num2str(Threshold1),'_thres2_',num2str(Threshold2),'.mat'],...
     ...
     'DSPs','BadChs','Temporal','LDA','Updated1','Updated2','UpDistance1','UpDistance2','DSPtotal','Lambdas','Thresholds')
