function ac = eeg_autocorr_welch_MJ(act, param, pct_data)
...
if pct_data ~=100
    ...
    temp = reshape(act(:, index, :), [ncomp size(index) .* [1 1]]);
    segments = temp(:, :, subset);
else
    segments = reshape(act(:, index, :), [ncomp size(index) .* [1 1]]);
end
...
% normalize
if pnts < param.Fs
    ac = [ac(:, 1:pnts, :) ./ (ac(:, 1) * (n_points:-1:1) / (n_points)) ...
        zeros(ncomp, param.Fs - n_points + 1)];
else
    ac = ac(:, 1:param.Fs + 1, :) ./ ...
        (ac(:, 1) * [(n_points:-1:n_points - param.Fs + 1) ...
        max(1, n_points - param.Fs)] / (n_points));
end

% resample to 1 second at 100 samples/sec
if ~exist('OCTAVE_VERSION', 'builtin') && exist('resample')
    ac = resample(double(ac'), 100, param.Fs)';
else
    ac = myresample(double(ac'), 100, param.Fs)';
end
ac = ac(:, 2:101);
