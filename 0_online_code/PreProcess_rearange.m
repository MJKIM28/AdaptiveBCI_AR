function [signal,trigger,param] = PreProcess(signal,trigger,param)
% signal [double]: time x ch, unit:uV

% 0.5Hz HPF
[signal] = prep_filter(signal,param.prep.filter.H_05);

% bad channel rejection
[signal, param.prep.badchrej] = prep_badchrej(signal,param.prep.badchrej,param.chaninfo); 

% rereference
[signal, param.prep.reref] = prep_reref(signal,param.prep.reref); 

% 50Hz LPF
[signal] = prep_filter(signal,param.prep.filter.L_50); 

% artifact rejection (ASR)
[signal, param.prep.ASR] = prep_ASR(signal,param.prep.ASR); 

% 12Hz LPF
[signal] = prep_filter(signal,param.prep.filter.L_12); 
