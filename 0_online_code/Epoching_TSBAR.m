function [EP,param]     = Epoching_TSBAR(sig, trig, param)
% Epoching_TSBAR ver.2
try
    ...
    if strcmp(param.decoder.mode,'training')
        ...
        singleTr = cell(param.NumStims,param.NumRepeat);
    elseif strcmp(param.decoder.mode,'testing')
...
%     START([32]) = []; %end_t([25 26])= [];
    ...
    tmp = NaN(param.NumCh, param.Totalepoc,param.repeat,param.NumStims,numTr);
    
    for b = 1 : numTr
        if START(b) ~= 0
            %         if b < numTr
            epoc_ind            = latency(START(b)+1 : end_t(b)-1);
            Diff = diff(epoc_ind)/500;
            SumDiff = movsum(Diff,2);
            DelayedIdx = find(Diff < 0.15);
            IsDelayed = SumDiff(DelayedIdx) < 0.6;
            epoc_ind(DelayedIdx(IsDelayed)) = NaN;
...
%              (latency(end_t(b)) - latency(START(b)))./500
            ...
            %         if length(epoc_ind) > 50
            %             epoc_ind            = latency(START(b)+2 : START(b)+40);
            %             stim_type           = type(START(b)+2 : START(b)+40);
            %         end
            %         else
            %          epoc_ind            = latency(START(b)+2 : end);
            %         stim_type           = type(START(b)+2 : end);
            %         end
